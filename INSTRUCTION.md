...
- **Product:** Sitemyra — multi-tenant website change-detection and uptime-monitoring SaaS.
...
- [ ] Code is merged to `main` with tests passing:
      `docker compose run --rm celery-browser-worker python manage.py test accounts monitors notifications billing common` (220 tests) and `cd frontend && npx tsc --noEmit`
